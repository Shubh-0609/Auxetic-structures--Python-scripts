...
# Model_unitcell=mdb.models[ModelName]
#            part generation
...
s = mdb.models['6_arrow'].ConstrainedSketch(name='Arrow1', 
sheetSize=200.0)
phi=math.pi/stars
...
p = mdb.models['6_arrow'].Part(name='Part-1', dimensionality=TWO_D_PLANAR, 
        type=DEFORMABLE_BODY)
p = mdb.models['6_arrow'].parts['Part-1']
p.BaseShell(sketch=s)

# Arrow-2
arrow_2=np.load('arrow_2.npy')
s1 = mdb.models['6_arrow'].ConstrainedSketch(name='Arrow2', 
sheetSize=200.0)
...
p = mdb.models['6_arrow'].Part(name='Part-2', dimensionality=TWO_D_PLANAR, 
        type=DEFORMABLE_BODY)
p = mdb.models['6_arrow'].parts['Part-2']
p.BaseShell(sketch=s1)

# Arrow-3
arrow_3=np.load('arrow_3.npy')
s2 = mdb.models['6_arrow'].ConstrainedSketch(name='Arrow3', 
sheetSize=200.0)
...
p = mdb.models['6_arrow'].Part(name='Part-3', dimensionality=TWO_D_PLANAR, 
        type=DEFORMABLE_BODY)
p = mdb.models['6_arrow'].parts['Part-3']
p.BaseShell(sketch=s2)

# Arrow-4
arrow_4=np.load('arrow_4.npy')
s3 = mdb.models['6_arrow'].ConstrainedSketch(name='Arrow4', 
sheetSize=200.0)
...
p = mdb.models['6_arrow'].Part(name='Part-4', dimensionality=TWO_D_PLANAR, 
        type=DEFORMABLE_BODY)
p = mdb.models['6_arrow'].parts['Part-4']
p.BaseShell(sketch=s3)

# Arrow-5
arrow_5=np.load('arrow_5.npy')
s4 = mdb.models['6_arrow'].ConstrainedSketch(name='Arrow5', 
sheetSize=200.0)
...
z=len(arrow_5)
for i in range(z-1):
    s4.Line(point1=(x[i],y[i]),point2=(x[i+1],y[i+1]))
p1 = mdb.models['6_arrow'].Part(name='Part-5', dimensionality=TWO_D_PLANAR, 
        type=DEFORMABLE_BODY)
p1.BaseShell(sketch=s4)

# Arrow-6
arrow_6=np.load('arrow_6.npy')
s5 = mdb.models['6_arrow'].ConstrainedSketch(name='Arrow6', 
sheetSize=200.0)
...
z=len(arrow_6)
for i in range(z-1):
    s5.Line(point1=(x[i],y[i]),point2=(x[i+1],y[i+1]))
p1 = mdb.models['6_arrow'].Part(name='Part-6', dimensionality=TWO_D_PLANAR, 
        type=DEFORMABLE_BODY)
p1.BaseShell(sketch=s5)



    # VIRTUAL POINTS 
Part_VirtPoint_X = mdb.models['6_arrow'].Part(dimensionality=TWO_D_PLANAR, name='VirtPoint_X', type=DEFORMABLE_BODY)
Part_VirtPoint_X.ReferencePoint(point=(0.0, 0.0, 0.0))

Part_VirtPoint_Y = mdb.models['6_arrow'].Part(dimensionality=TWO_D_PLANAR, name='VirtPoint_Y', type=DEFORMABLE_BODY)
Part_VirtPoint_Y.ReferencePoint(point=(0.0, 0.0, 0.0))
    

 #               Assembly and instances with merge function  
a = mdb.models['6_arrow'].rootAssembly
...
p = mdb.models['6_arrow'].parts['Part-5']
a.Instance(name='Part-5-1', part=p, dependent=ON)
p = mdb.models['6_arrow'].parts['Part-6']
a.Instance(name='Part-6-1', part=p, dependent=ON)
a = mdb.models['6_arrow'].rootAssembly
Partins=a.InstanceFromBooleanMerge(name='Arrow_6_Part', instances=(
    a.instances['Part-1-1'], a.instances['Part-2-1'], 
    a.instances['Part-3-1'],a.instances['Part-4-1'],a.instances['Part-5-1'],a.instances['Part-6-1'], ), 
...
# CUTS
# Upper
s6 = mdb.models['6_arrow'].ConstrainedSketch(name='Arrow', 
sheetSize=200.0)
g, v, d, c = s6.geometry, s6.vertices, s6.dimensions, s6.constraints
p1 = mdb.models['6_arrow'].parts['Arrow_6_Part']
cut_1=np.load('cut_1.npy')
x=cut_1[:,0]
y=cut_1[:,1]
z=len(cut_1)
for i in range(z-1):
    s6.Line(point1=(x[i],y[i]),point2=(x[i+1],y[i+1]))

p1.Cut(sketch=s6)
s6.unsetPrimaryObject()

# Lower
s7 = mdb.models['6_arrow'].ConstrainedSketch(name='Arrow', 
sheetSize=200.0)
g, v, d, c = s7.geometry, s7.vertices, s7.dimensions, s7.constraints
p1 = mdb.models['6_arrow'].parts['Arrow_6_Part']
cut_2=np.load('cut_2.npy')
x=cut_2[:,0]
y=cut_2[:,1]
z=len(cut_2)
for i in range(z-1):
    s7.Line(point1=(x[i],y[i]),point2=(x[i+1],y[i+1]))
p1.Cut(sketch=s7)
s7.unsetPrimaryObject()

# Right
s8 = mdb.models['6_arrow'].ConstrainedSketch(name='Arrow', 
sheetSize=200.0)
g, v, d, c = s8.geometry, s8.vertices, s8.dimensions, s8.constraints
s8.setPrimaryObject(option=SUPERIMPOSE)
p1 = mdb.models['6_arrow'].parts['Arrow_6_Part']
s8.rectangle(point1=(((h+l*(math.sin(theta)/math.tan(phi))- l * math.cos(theta))-thick2)*math.sin(2*phi), (h+l*(math.sin(theta)/math.tan(phi))- l * math.cos(theta))+thick2), point2=(((h+l*(math.sin(theta)/math.tan(phi))- l * math.cos(theta))+thick2)*math.sin(2*phi), -(h+l*(math.sin(theta)/math.tan(phi))- l * math.cos(theta))-thick2))
p1.Cut(sketch=s8)
s8.unsetPrimaryObject() 

# Left
s9 = mdb.models['6_arrow'].ConstrainedSketch(name='Arrow', 
sheetSize=200.0)
g, v, d, c = s9.geometry, s9.vertices, s9.dimensions, s9.constraints
s9.setPrimaryObject(option=SUPERIMPOSE)
p1 = mdb.models['6_arrow'].parts['Arrow_6_Part']
s9.rectangle(point1=((-(h+l*(math.sin(theta)/math.tan(phi))- l * math.cos(theta))-thick2)*math.sin(2*phi), (h+l*(math.sin(theta)/math.tan(phi))- l * math.cos(theta))+thick2), point2=((-(h+l*(math.sin(theta)/math.tan(phi))- l * math.cos(theta))+thick2)*math.sin(2*phi), -(h+l*(math.sin(theta)/math.tan(phi))- l * math.cos(theta))-thick2))
p1.Cut(sketch=s9)
s9.unsetPrimaryObject() 

    #                Material assignment and section generation
mdb.models['6_arrow'].Material(name='Structure_Material')
mdb.models['6_arrow'].materials['Structure_Material'].Elastic(table=((
193000000000.0000, 0.33), ))
mdb.models['6_arrow'].materials['Structure_Material'].Density(table=((0.00836000, ), 
))
mdb.models['6_arrow'].HomogeneousSolidSection(name='Section_Star', 
...
Stepstatic = mdb.models['6_arrow'].StaticStep(description='Uniaxial compression in Y direction',
...
p = mdb.models['6_arrow'].parts['Arrow_6_Part']
p.SectionAssignment(offset=0.0, offsetField='', offsetType=MIDDLE_SURFACE, region=Region(faces=mdb.models['6_arrow'].parts['Arrow_6_Part'].faces.getByBoundingBox(-(h+l*(math.sin(theta)/math.tan(phi)))-1,-(h+l*(math.sin(theta)/math.tan(phi)))-1,-1,(h+l*(math.sin(theta)/math.tan(phi)))+1,(h+l*(math.sin(theta)/math.tan(phi)))+1,1)
...
p.Set(name='Down_Nodes', nodes=
    p.nodes.getByBoundingCylinder(( -thick2-x*math.cos(math.pi/6), -(h+l*(math.sin(theta)/math.tan(phi))- l * math.cos(theta))+x*math.sin(math.pi/6),0),(thick2/2+x*math.cos(math.pi/6), -(h+l*(math.sin(theta)/math.tan(phi))- l * math.cos(theta))+thick2/math.tan(math.pi/3) +x+x*math.sin(math.pi/6), 0),0.25) )  

p.Set(name='Up_Nodes', nodes=
    p.nodes.getByBoundingCylinder((-thick2-x*math.cos(math.pi/6), h+l*(math.sin(theta)/math.tan(phi))- l * math.cos(theta)-thick2/math.tan(math.pi/3)-x*math.sin(math.pi/6) -thick2, 0),( thick2/2+x*math.cos(math.pi/6), h+l*(math.sin(theta)/math.tan(phi))- l * math.cos(theta)+x*math.sin(math.pi/6)-thick2, 0),0.025) )    

p.Set(name='Left_Nodes', nodes=
    p.nodes.getByBoundingBox((-(h+l*(math.sin(theta)/math.tan(phi))- l * math.cos(theta))+thick2)*math.sin(2*phi)- epsilon*math.sin(2*phi),thick2-epsilon, 0, (-(h+l*(math.sin(theta)/math.tan(phi))- l * math.cos(theta))+thick2)*math.sin(2*phi) +epsilon*math.sin(2*phi),-thick2+epsilon, epsilon))    
...
p.Set(name='LowerMidVert_Line_Nodes', nodes=p.nodes.getByBoundingBox
    (-0.1/4.0, -(h+l*(math.sin(theta)/math.tan(phi))- l * math.cos(theta))- epsilon, -epsilon, 0.1/4.0, (h+l*(math.sin(theta)/math.tan(phi))- l * math.cos(theta)) + epsilon, epsilon))
...
Model_Arrow=mdb.models['6_arrow']
...
for i in p.sets['Right_Nodes'].nodes:
    ...
    mdb.models['6_arrow'].Equation(name='LR_Const_at_X_' + str(i.label), terms=(
        ( 1.0, 'Arrow_6_Part-1.Right_Node_Pair_' + str(i.label), 1), 
        (-1.0, 'Arrow_6_Part-1.Left_Node_Pair_' + str(i.label), 1), 
        (coeff1, 'VirtPointInst_X.VPoint_X', 1), 
        (coeff2, 'VirtPointInst_Y.VPoint_Y', 1)))
        
    # Y-COORDINATE OF DEPENDENT SET
    mdb.models['6_arrow'].Equation(name='LR_Const_at_Y_' + str(i.label), terms=(
        ( 1.0, 'Arrow_6_Part-1.Right_Node_Pair_' + str(i.label), 2), 
        (-1.0, 'Arrow_6_Part-1.Left_Node_Pair_' + str(i.label), 2), 
...
for i in p.sets['Up_Nodes'].nodes:
    ...
    # X-COORDINATE OF DEPENDENT SET
    mdb.models['6_arrow'].Equation(name='UD_Const_at_X_' + str(i.label), terms=(
        ( 1.0, 'Arrow_6_Part-1.Up_Node_Pair_' + str(i.label), 1), 
        (-1.0, 'Arrow_6_Part-1.Down_Node_Pair_' + str(i.label), 1), 
        (coeff1, 'VirtPointInst_X.VPoint_X', 1), 
        (coeff2, 'VirtPointInst_Y.VPoint_Y', 1)))
        
    # Y-COORDINATE OF DEPENDENT SET
    mdb.models['6_arrow'].Equation(name='UD_Const_at_Y_' + str(i.label), terms=(
        ( 1.0, 'Arrow_6_Part-1.Up_Node_Pair_' + str(i.label), 2), 
        (-1.0, 'Arrow_6_Part-1.Down_Node_Pair_' + str(i.label), 2), 
        (coeff1, 'VirtPointInst_X.VPoint_X', 2), 
        (coeff2, 'VirtPointInst_Y.VPoint_Y', 2)))
...
mdb.Job(atTime=None, contactPrint=OFF, description='', echoPrint=OFF, 
    ...
    memory=90, memoryUnits=PERCENTAGE, model='6_arrow', modelPrint=OFF, 
...
All_Elems = o1.rootAssembly.instances['ARROW_6_PART-1'].elementSets['ALL_ELEMS']
