...
def savearray(parameter):
    h,l,theta,thick1,thick2,stars=4,5,math.pi/3,0.5,1,4
    # h,l,theta,thick1,thick2,stars=4,5,math.pi/3.8,0.5,1,4
    ...
    sci = np.deg2rad(270)
    rotmat = np.array([[cos(sci), -sin(sci)], [sin(sci), cos(sci)]])
    arrow4 = np.dot(rotmat, arrow_1.T).T
    arrow_4 = np.array(arrow4)
...
for h in range(3, 5, 1):
    for l in range(2, 4, 1):
        for theta in np.arange(math.pi / 3.8, math.pi / 2.5, math.pi / 18):
            for thick1 in np.arange(0.25, 0.75, 0.25):
                for thick2 in np.arange(0.25, 0.75, 0.25):
                    for stars in range(4, 5):
                        try:
                            t = time.localtime()
                            ...
                            parameter = [h, l, theta, thick1, thick2, stars]
                            savearray(parameter)
                            # abaq='abaqus cae script=4_arrow_cae_quad_dominated.py'
                            abaq = 'abaqus cae noGUI=4_arrow_cae_quad_dominated.py'
                            d = os.system(abaq)
                            print(d)
                            print(parameter)
                            t = time.localtime()
