...
def genAlines(h, l, theta, thick1, thick2, stars):
    ...
    b1bx = np.linspace(l * (math.sin(theta)) - t * math.sin(phi), l * (math.sin(theta)) + thick1 * math.sin(phi),
                       lpoints)
    b1by = np.linspace(l * (math.sin(theta) / math.tan(phi)) - t * math.cos(phi),
                       l * (math.sin(theta) / math.tan(phi)) + thick1 * math.cos(phi), lpoints)
...
def savearray(parameter):
    # h,l,theta,thick1,thick2,stars=4,3,math.pi/2.5,0.25,0.5,3
...
for h in range(3, 5, 1):
    for l in range(2, 4, 1):
        for theta in np.arange(math.pi / 2.8, math.pi / 2.5, math.pi / 18):
            for thick1 in np.arange(0.25, 0.75, 0.25):
                for thick2 in np.arange(0.25, 0.75, 0.25):
                    for stars in range(3, 4):
                        try:
                            t = time.localtime()
                            ...
                            parameter = [h, l, theta, thick1, thick2, stars]
                            savearray(parameter)
                            abaq='abaqus cae script=3_arrow_cae.py'
                            # abaq = 'abaqus cae noGUI=3_arrow_cae.py'
                            d = os.system(abaq)
                            print(d)
                            print(parameter)
                            t = time.localtime()
